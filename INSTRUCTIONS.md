...
Another thing that negatively impacts the performance of our application is
what's called the "waterfall effect." This gets its name from the "waterfall"
view in the browser's Network developer tools which shows when the browser
starts requesting a certain asset and when the request for that asset finishes.
There are some assets we need to load before the app can render the initial
page. The earlier we can start requesting those assets, the faster we can show
the user their data.

Both of these optimizations are intended to improve what's called the
...
You may notice that the profiling report is a bit noisy. React re-renders a
_lot_ and it can be difficult to make much sense of that profiling report. For
this reason, the React team is introducing an _experimental_ tracing API:

📜 [Interaction tracing with React](https://fb.me/react-interaction-tracing)

Go ahead and add interaction tracing to a few spots in your app. In my final
version, I put the tracing import into the `src/components/profile.js` module so
I don't have to import the `unstable_trace` function from `schedular/tracing`,
but you feel free to do this however you like.

Let's add this tracing to the status buttons (the "Mark as Read" etc. buttons
you find next to the books). Add an interaction trace for the `handleClick`
function.

Once you've finished that, keep in mind: that interaction is async, so you'll
also need `unstable_wrap` as well. So you'll have the `trace` in the
`status-buttons.js` module and the wrap in the `useAsync` hook (in the
`src/utils/use-async.js` module).

Once you have this in place, go ahead and use the React DevTools profiler and
you'll notice that these interactions show up in the "interactions" tab as well!

**Files:**

- `src/components/profiler.js`
- `src/components/status-buttons.js`
- `src/utils/use-async.js`
...
https://ws.kcd.im/?ws=Build%20React%20Apps%20%F0%9F%93%9A&e=09%3A%20Performance&em=
